...
class TwoLayerNet(object):
  ...
  def train(self, X, y, X_val, y_val,
    ...
    for it in range(num_iters):
      ...
      if it % iterations_per_epoch == 0:
        # Check accuracy
        train_acc = np.mean((self.predict(X_batch) == y_batch), dtype=np.float64)
        val_acc = np.mean((self.predict(X_val) == y_val), dtype=np.float64)
  ...
  def predict(self, X):
    ...
    scores = hid.dot(W2) + b2
    exp_scores = np.exp(scores-np.expand_dims(np.max(scores, axis=1), axis=1))
    y_pred = np.argmax(exp_scores/np.expand_dims(np.sum(exp_scores, axis=1), axis=1), axis=1)
    ###########################################################################
    #                              END OF YOUR CODE                           #
    ###########################################################################
